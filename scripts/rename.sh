...
if [ -z "$(which rename)" ]; then
    echo "\"rename\" is missing."
    echo "You can install it via your package manager or via \"brew install rename\""
    exit 1
fi
...
find . -name Dockerfile -exec sed -e "s#${old_package_name}#${new_package_name}#" -e "s#${old_binary_name}#${new_binary_name}#" -i '{}' \;
